...
# Load all files from /etc/bashrc.d
...
install_claude_code() {
	echo "# install_claude_code()"
	local version=$1
	verify_version_is_latest "$version"

	# Install claude code https://code.claude.com/docs/en/setup
	# NOTE this installs in the roots HOME dir, currently there is no option to install it
	# 	'system wide' via the provided script
	ccurl https://claude.ai/install.sh | bash

	# In order to install it 'system wide' (i.e. in /usr/local) such that all users can utilize it,
	# the install folder is moved.
	# NOTE: this might break in future versions, if claude decides to move the install dir
	mv /root/.local/share/claude /usr/local/share/claude

	# Next, remove the old simlink and create a new one
	# NOTE: this might break in future versions, if claude decides to do this differently
	version_installed=$(readlink /root/.local/bin/claude | xargs basename)
    rm /root/.local/bin/claude
    ln -sf "/usr/local/share/claude/versions/${version_installed}" /usr/local/bin/claude
}
...
create_user() {
	...
	# Copy /tmp/USER/ into the new user's home dir if it exists
	if [ -d "/tmp/USER" ]; then
		cp -r /tmp/USER/. "/home/$username/"
